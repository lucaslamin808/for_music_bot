...
class AudioController(object):
    """ Controlador de todas a funções para reproduzir músicas
    ...
    def next_song(self, error):
        # Executada após uma música acabar para reproduzir a próxima automaticamente
    ...
    async def play_song(self, song):
        # Reproduz o objeto que contém a música

        if self.playlist.loop != True: # desliga o timer caso não esteja em loop
            self.timer.cancel()
            self.timer = utils.Timer(self.timeout_handler)
    ...
    async def process_song(self, track):
        # Adiciona a música a fila e a reproduz caso seja a primeira
        ...
        if is_playlist != linkutils.Playlist_Types.Unknown:
            ...
            if self.current_song == None:
                ...
                print("Reproduzindo {}".format(track))
    ...
    def search_youtube(self, title):
        # Pesquisa no youtube pelo título do vídeo e retorna o link do primeiro resultado

        # caso o usuário insira o link direto
    ...
    async def stop_player(self):
        # Para a reprodução e remove todas as músicas da fila
    ...
    async def prev_song(self):
        # Carrega a última música que foi reproduzida e a reproduz novamente
    ...
    async def timeout_handler(self):
        ...
        if sett.get('vc_timeout') == False:
            self.timer = utils.Timer(self.timeout_handler)  # redefine o timer
            return
