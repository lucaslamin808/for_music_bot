...
class Settings():
    ...
    async def format(self):
        embed = discord.Embed(
            title="Configurações", description=self.guild.name, color=config.EMBED_COLOR)

        embed.set_thumbnail(url=self.guild.icon_url)
        embed.set_footer(
            text="Use {}set nome da configuração valor".format(config.BOT_PREFIX))
    ...
    async def default_nickname(self, setting, value, ctx):
        ...
        if len(value) > 32:
            await ctx.send("`Erro: o nick excede o limite de caracteres`\nUse: {}set {} nickn".format(config.BOT_PREFIX, setting))
            return False
        else:
            ...
            try:
                ...
                await ctx.send("`Erro: Não foi possível alterar o nick.")
    ...
    async def command_channel(self, setting, value, ctx):
        ...
        for chan in self.guild.text_channels:
            if chan.name.lower() == value.lower():
                ...
                found = True
        if found == False:
            await ctx.send("`Erro: Canal não encontrado`\nUse: {}set {} nome do canal".format(config.BOT_PREFIX, setting))
            return False
    ...
    async def start_voice_channel(self, setting, value, ctx):
        ...
        for vc in self.guild.voice_channels:
            if vc.name.lower() == value.lower():
                ...
                found = True
        if found == False:
            await ctx.send("`Erro: Canal de voz não encontrado`\nUse: {}set {} nome do canal de voz".format(config.BOT_PREFIX, setting))
            return False
    ...
    async def user_must_be_in_vc(self, setting, value, ctx):
        if value.lower() == "true":
            ...
            await ctx.send("`Erro: O valor deve ser True/False`\nUse: {}set {} True ou False".format(config.BOT_PREFIX, setting))
            return False
    ...
    async def button_emote(self, setting, value, ctx):
        ...
        if emoji is None:
            await ctx.send("`Erro: Nome de emote não encontrado`\nUse: {}set {} emote".format(config.BOT_PREFIX, setting))
            return False
        else:
            self.config[setting] = value
    ...
    async def default_volume(self, setting, value, ctx):
        try:
            ...
            await ctx.send("`Erro: O valor deve ser um número`\nUse: {}set {} 0-100".format(config.BOT_PREFIX, setting))
            return False
    ...
    async def vc_timeout(self, setting, value, ctx):
        ...
        if config.ALLOW_VC_TIMEOUT_EDIT == False:
            await ctx.send("`Erro: Esse valor não pode ser modificado!".format(config.BOT_PREFIX, setting))
        ...
        if value.lower() == "true":
            ...
            await ctx.send("`Erro: O valor deve ser True/False`\nUse: {}set {} True/False".format(config.BOT_PREFIX, setting))
            return False
